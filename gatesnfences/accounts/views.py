...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('/accounts/login/?action=verification&email=' + email)
...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.error(request, 'Wrong User credentials🙅‍♂️')
            return redirect('login')
...
def forgotPassword(request):
    ...
    return render(request, template_name='accounts/forgot_password.html')
...
def reset_password_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.error(request, 'The reset link has expired🤦‍♂️')
        return redirect('login')
